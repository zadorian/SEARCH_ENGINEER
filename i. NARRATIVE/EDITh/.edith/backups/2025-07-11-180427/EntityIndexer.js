...
export class EntityIndexer {
    ...
    async scheduleHeavyPass() {
        console.log('[EntityIndexer] scheduleHeavyPass called');
        if (this.isIndexing) {
            console.log('[EntityIndexer] Already indexing, skipping');
            return;
        }
        
        const editor = document.getElementById('main-text-editor');
        if (!editor) {
            console.log('[EntityIndexer] No editor found');
            return;
        }
        
        const text = editor.value || editor.textContent || '';
        console.log('[EntityIndexer] Text length:', text.length);
    ...
    async performHeavyPass(text) {
        console.log('[EntityIndexer] performHeavyPass starting');
        this.isIndexing = true;
        
        try {
            const model = this.selectHeavyPassModel(text);
            console.log('[EntityIndexer] Using model:', model);
            const entities = await this.extractEntitiesHeavy(text, model);
            console.log('[EntityIndexer] Extracted entities:', entities.length);
...
Focus on clear, unambiguous entities. Be conservative.`;
    ...
    async extractEntitiesHeavy(text, model) {
        console.log('[EntityIndexer] extractEntitiesHeavy called with model:', model);
...
Provide rich metadata where applicable.`;
    ...
    async callEntityAPI(systemPrompt, text, model) {
        console.log('[EntityIndexer] Calling entity extraction API');
        ...
        console.log('[EntityIndexer] API response:', data);
    ...
    notifyIndexUpdate() {
        console.log('[EntityIndexer] notifyIndexUpdate called');
        console.log('[EntityIndexer] Entity count:', this.entityGraph.size);
        console.log('[EntityIndexer] LeftRail available:', !!this.app.leftRail);
