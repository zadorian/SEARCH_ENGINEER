...
class EdgarSearchIntegration:
    ...
    def __init__(self, output_dir: str = None):
        ...
        Args:
            output_dir: Directory for output files (defaults to EDGAR-main/output)
        """
        self.project_root = Path("/Users/attic/Library/Mobile Documents/com~apple~CloudDocs/01_ACTIVE_PROJECTS/Sastre-Black/Corporate_Search")
        self.edgar_dir = self.project_root / "EDGAR-main"
        
        if output_dir:
            self.output_dir = Path(output_dir)
        else:
            self.output_dir = self.edgar_dir / "output"
