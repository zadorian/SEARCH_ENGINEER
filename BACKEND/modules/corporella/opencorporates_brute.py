...
# Direct import of the gemini module - this should work whether run directly or from BRUTE.py
...
class OpenCorporatesAPI:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        # If still no API key, try to load from config (if available)
        if not self.api_key:
            try:
                from config import init_config
                config = init_config()
                self.api_key = config.get('OPENCORPORATES_API_KEY')
            except ImportError:
                # If config module isn't available, continue without API key
                pass
