...
async def main():
    ...
    async def crawl_domain(seed, idx, total):
        """Crawl single domain and index or output to JSON."""
        print(f"\n[{idx}/{total}] Crawling {seed}...", file=sys.stderr)
        ...
        elif not es and results:
            # Output to stdout as JSONL when --no-index
            import json
            for result in results:
                print(json.dumps(result, default=str))
            sys.stdout.flush()
            indexed = len(results)  # Mark as "indexed" to file
