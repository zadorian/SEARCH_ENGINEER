...
**Metadata Location:** `../../metadata/C-2/sources/`
...
```bash
curl "http://localhost:9200/C-2/_search?q=cryptocurrency&size=10"
```

### 2. Filter by Source Type
```bash
# Only books
curl "http://localhost:9200/C-2/_search" -H 'Content-Type: application/json' -d '{
  "query": {
    "bool": {
      "must": [
...
```bash
curl "http://localhost:9200/C-2/_search?size=0" -H 'Content-Type: application/json' -d '{
  "aggs": {
    "by_source": {
...
```typescript
// Get document
const doc = await getDoc('C-2', docId);
...
```typescript
// Get entity from C-1
const entity = await getEntity('C-1-project-{id}', entityId);
...
### Add New Document
```bash
curl -X POST "http://localhost:9200/C-2/_doc" -H 'Content-Type: application/json' -d '{
  "source_type": "report",
  "title": "New Report",
  "content": "Full text...",
  "indexed_at": "2025-12-03T..."
...
- **C-1 Integration:** `../C-1/README.md`
- **Source Breakdown:** `../../metadata/C-2/SOURCES_README.md`
- **YouTube Source:** `../../metadata/C-2/sources/youtube/metadata.json`
- **Books Source:** `../../metadata/C-2/sources/books/metadata.json`
- **Reports Source:** `../../metadata/C-2/sources/reports/metadata.json`
- **Entity Extraction:** `../../server/services/C-2/EntityExtractor.ts`
- **C-2 Metadata:** `../../metadata/C-2/metadata.json`
