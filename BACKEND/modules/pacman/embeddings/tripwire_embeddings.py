...
Generates 1024-dimensional embeddings using intfloat/multilingual-e5-large (StandardEmbedder) for:
...
# Model config - Sastre Standard (1024D)
MODEL_NAME = "intfloat/multilingual-e5-large"
EMBEDDING_DIM = 1024


def load_model():
    """Load the Sastre standard embedder (1024D e5-large)."""
    try:
        import sys
        if '/data' not in sys.path:
            sys.path.insert(0, '/data')
        from shared.embedders import get_embedder
        logger.info(f"Loading Sastre StandardEmbedder: {MODEL_NAME}")
        embedder = get_embedder()
        logger.info(f"StandardEmbedder loaded: {embedder.dimensions}D on {embedder.model.device}")
        return embedder
    except ImportError as e:
        logger.error(f"StandardEmbedder not available: {e}")
        raise
...
def embed_category(model, category: Dict) -> Dict:
    ...
    embeddings = model.encode_passage(texts, convert_to_tensor=True, show_progress_bar=False)
