...
try:
    from brute.categorizer.intelligent_categorizer import IntelligentCategorizer
except ImportError:
    print("⚠️  Could not import IntelligentCategorizer. Sector categorization will be skipped.")
    IntelligentCategorizer = None
...
def build_document(record: dict, decision: TierDecision, content: Optional[str] = None, categorizer: Optional[Any] = None) -> dict:
    ...
    # Apply Intelligent Categorization (Sectors)
    if categorizer:
        try:
            # Create a record structure compatible with categorize_result
            cat_input = {
                "url": record.get("url"),
                "domain": record.get("domain"),
                "content": content or "", # Use full content if available for embeddings
                "title": "", # Pacman docs might not have titles extracted yet
                "snippet": (content or "")[:1000] # Use first 1000 chars as snippet
            }
            enriched = categorizer.categorize_result(cat_input)
            
            doc["metadata"]["category"] = enriched.get("category")
            doc["metadata"]["category_confidence"] = enriched.get("category_confidence")
            doc["metadata"]["category_evidence"] = enriched.get("category_evidence")
...
class PacmanRunner:
    ...
    def __init__(self, source_id: str = SOURCE_ID):
        ...
        # Initialize Intelligent Categorizer
        self.categorizer = None
        if IntelligentCategorizer:
            try:
                self.categorizer = IntelligentCategorizer()
                print("🧠 IntelligentCategorizer initialized for sector analysis")
            except Exception as e:
                print(f"⚠️  Failed to init IntelligentCategorizer: {e}")
    ...
    async def run(
        ...
        async for doc in documents:
            ...
            try:
                ...
                # Build document
                indexed_doc = build_document(doc, decision, content if decision.tier == Tier.FULL else None, self.categorizer)
