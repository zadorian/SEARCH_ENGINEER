...
Uses vector embeddings (intfloat/multilingual-e5-large via StandardEmbedder) to match ANY user input to canonical
...
def _get_model():
    """Lazy load the standard embedder (1024D e5-large)."""
    global _model
    if _model is None:
        try:
            import sys
            ...
            _model = get_embedder()
            logger.info("Loaded StandardEmbedder (1024D multilingual-e5-large)")
        except ImportError as e:
            logger.warning(f"StandardEmbedder not available ({e}), falling back to alias matching")
