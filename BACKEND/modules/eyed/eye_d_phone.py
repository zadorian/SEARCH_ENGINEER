...
class EyeDPhoneEngine:
    ...
    def search(self) -> List[Dict[str, Any]]:
        ...
        try:
            results = loop.run_until_complete(self.handler.search_phone(self.keyword))
            web_results: List[Dict[str, Any]] = []
            for i, r in enumerate(results.get('results', []), 1):
                data = r.get('data', {})
                snippet = ' | '.join([f"{k}: {str(v)[:80]}" for k, v in list(data.items())[:8]]) if isinstance(data, dict) else str(data)[:300]
                web_results.append({
                    'url': f"https://truecaller.com/search/{self.keyword.replace('+','%2B')}",
                    'title': f"Phone: {self.keyword} (from {r.get('source','unknown')})",
                    'snippet': snippet,
                    'engine': self.name,
