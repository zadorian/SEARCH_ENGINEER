...
NODE_API_BASE_URL = (
    os.getenv("NODE_API_BASE_URL")
    or os.getenv("NODE_SERVER_URL")
    or os.getenv("NODE_SERVER")
    or os.getenv("PYTHON_API_URL")
    or "http://localhost:3000"
)
PYTHON_API_BASE_URL = os.getenv("PYTHON_API_BASE_URL") or os.getenv("PYTHON_API_URL") or "http://localhost:8000"
ES_URL = os.getenv("ELASTICSEARCH_URL", "http://localhost:9200")


# =============================================================================
# CYMONIDES BRIDGE (Unknown Knowns Check + Graph Management)
# =============================================================================

class CymonidesBridge:
    """
    Bridge to Cymonides entity storage, WDC indices, and Graph persistence.

    Uses:
    - wdc-person-entities: Person records from Schema.org
    - wdc-organization-entities: Company/org records
    - cymonides-1-{project}: Project-specific graph
    """
    ...
    def _lazy_load(self):
        ...
        try:
            from DEFINITIONAL.wdc_query import WDCQueryService, search_person_entities, search_organization_entities
    ...
    async def _get_es(self):
        if not self._es and AsyncElasticsearch:
    ...
    async def create_node(
        ...
        """
        Create a node in the project-specific Cymonides index.
        """
        ...
        node_id = f"{node_class[:3]}_{hashlib.md5(label.encode()).hexdigest()[:12]}"
        ...
        await es.index(index=index, id=node_id, document=doc, refresh="wait_for")
        return node_id
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    # Fallback if LINKLATER is in path
                    from api import get_linklater
...
class WatcherBridge:
    ...
    def __init__(self, base_url: str = NODE_API_BASE_URL):
        base_url = (base_url or NODE_API_BASE_URL).rstrip("/")
        # Normalize accidental "/api" base URLs so we don't double-prefix
        # routes like "/api/api/trpc/...".
        if base_url.endswith("/api"):
            base_url = base_url[:-4]
        self.base_url = base_url
        self._session = None
        # Auto-footnote state (document_id -> next footnote number)
        self._footnote_counters: Dict[str, int] = {}
        self._footnote_lock = asyncio.Lock()
    ...
    async def get_sections(self, document_id: str) -> List[Dict[str, Any]]:
        ...
        """
        candidates = [
            f"{self.base_url}/api/narratives/documents/{document_id}/sections",
            f"{self.base_url}/narratives/documents/{document_id}/sections",  # legacy
        ]
        ...
        if last_error:
            print(f"Get sections error: {last_error}")
        return []
    ...
    async def update_section(
        ...
        if last_error:
            print(f"Update section error: {last_error}")
        return False

    async def list_footnotes(self, document_id: str) -> List[Dict[str, Any]]:
        """List footnotes for a narrative document."""
        candidates = [
            f"{self.base_url}/api/narratives/documents/{document_id}/footnotes",
            f"{self.base_url}/narratives/documents/{document_id}/footnotes",  # legacy
        ]
    ...
    async def _next_footnote_number(self, document_id: str) -> int:
        """Return the next available footnote number for a document (cached)."""
        async with self._footnote_lock:
            next_num = self._footnote_counters.get(document_id)
            if next_num is None:
                existing = await self.list_footnotes(document_id)
                max_existing = 0
                for fn in existing:
                    try:
                        num = int(fn.get("number") or 0)
                    except Exception:
                        num = 0
                    if num > max_existing:
                        max_existing = num
            ...
            self._footnote_counters[document_id] = next_num + 1
            return next_num
    ...
    async def create_footnote(
        ...
        """
        candidates = [
            f"{self.base_url}/api/narratives/documents/{document_id}/footnotes/create",
            f"{self.base_url}/narratives/documents/{document_id}/footnotes/create",  # legacy
        ]
        ...
        if last_error:
            print(f"Create footnote error: {last_error}")
        return None
    ...
    async def stream_finding_to_section(
        ...
        if source_url:
            if footnote_number is None:
                footnote_number = await self._next_footnote_number(document_id)

            marker = f"[^{footnote_number}]"
            if marker not in content:
                content = f"{content} {marker}"

            # URL-only footnotes (no access dates / source names).
            await self.create_footnote(
                document_id=document_id,
                footnote_number=footnote_number,
                footnote_content=source_url,
                source_url=source_url,
            )

        return await self.update_section(document_id, section_title, content, operation="append")
...
class IOBridge:
    ...
    async def execute(
        ...
        if self._executor:
            try:
                result = await self._executor.execute(entity_type, value, jurisdiction)
                # Convert IOResult to dict if necessary
