...
outputs to news_entities_<COUNTRY>_<timestamp>.json.
...
def _sanitize_label(label: str) -> str:
    cleaned = re.sub(r"[^A-Za-z0-9_-]+", "-", label).strip("-_")
...
def _build_output_path(output_path: Optional[Path], label: Optional[str]) -> Path:
    timestamp = _timestamp()
    label_part = f"_{_sanitize_label(label)}" if label else ""
    if output_path:
        if output_path.exists() and output_path.is_dir():
            path = output_path / f"{OUTPUT_BASENAME}{label_part}_{timestamp}.json"
        else:
            suffix = output_path.suffix or ".json"
            path = output_path.with_name(f"{output_path.stem}{label_part}_{timestamp}{suffix}")
    else:
        path = OUTPUT_DIR / f"{OUTPUT_BASENAME}{label_part}_{timestamp}.json"
...
class EntityHarvester:
    ...
    async def harvest_keyword(self, keyword: str, jurisdiction: str) -> int:
        ...
        try:
            ...
            pages_fetched = results.get("total_pages_fetched", 0)
    ...
    async def harvest_jurisdiction(
        ...
        self._current_jurisdiction = jur
    ...
    def _output_label(self) -> str:
        jurs: List[str] = []
        for jur in self.stats.jurisdictions_processed:
            if jur and jur not in jurs:
                jurs.append(jur)

        if self._current_jurisdiction and self._current_jurisdiction not in jurs:
            jurs.append(self._current_jurisdiction)

        if not jurs:
            return "UNSPEC"
        if len(jurs) == 1:
            return jurs[0]
        if set(jurs) == set(EU27_UK):
            return "EU27_UK"
        return "-".join(jurs)
    ...
    def save_results(self, output_path: Optional[Path] = None):
        ...
        path = _build_output_path(output_path, label=label)
...
async def main():
    ...
    parser.add_argument(
        "--output",
        "-o",
        help="Output file path or directory (country-labeled, timestamped, never overwrites existing files)",
    )
