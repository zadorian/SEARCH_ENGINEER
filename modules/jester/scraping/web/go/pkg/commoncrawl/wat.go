...
func InitImport(archiveName string) ([]WatSegment, error) {
	...
	fmt.Printf("⬇️  Downloading segments file: %s\n", url)
	...
	if err != nil {
		return segmentList, fmt.Errorf("failed to download segments file: %v", err)
	}
	defer resp.Body.Close()
	fmt.Println("✅ Download complete. Extracting segments...")

	// extract gzip
	gr, err := gzip.NewReader(resp.Body)
	if err != nil {
		return segmentList, fmt.Errorf("failed to create gzip reader: %v", err)
	}
	defer gr.Close()
	...
	fmt.Printf("✅ Found %d segments. Processing file lists...\n", len(segments))
...
// ParseWatLine - public wrapper for readPageContent for use by outlinker
func ParseWatLine(line string) *WatPage {
	// First, extract the URL record from the line
	sourceURLRecord := extractURLFromLine(line)
	if sourceURLRecord == nil {
		return nil
	}
	
	return readPageContent(line, sourceURLRecord)
}

// extractURLFromLine - extract URL record from WAT line for public use
func extractURLFromLine(line string) *URLRecord {
	parsedJSON := gjson.Parse(line)
	
	// Extract the source URL
	sourceURL := parsedJSON.Get("Envelope.WARC-Header-Metadata.WARC-Target-URI").String()
	if sourceURL == "" {
		return nil
	}
	
	sourceURLRecord := &URLRecord{}
	if !buildURLRecord(sourceURL, sourceURLRecord) {
		return nil
	}
	
	return sourceURLRecord
}

// IsSubdomain - check if this URLRecord represents a subdomain
func (u *URLRecord) IsSubdomain() bool {
	if u.Host == "" || u.Domain == "" {
		return false
	}
	
	// Count dots in the host vs domain
	hostDots := strings.Count(u.Host, ".")
	domainDots := strings.Count(u.Domain, ".")
	
	// If host has more dots than domain, it's likely a subdomain
	return hostDots > domainDots
}
...
// watFileNumberRegex - precompiled regex for extracting file number
var watFileNumberRegex = regexp.MustCompile(`-(\d+)\.warc\.wat\.gz$`)
...
func ExtractWatFileNumber(filename string) (string, error) {
	matches := watFileNumberRegex.FindStringSubmatch(filename)
