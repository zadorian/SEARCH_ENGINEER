...
class DrillConfig:
    ...
    # Geolocation targeting (Browser API)
    # Dict with lat, lon, distance, strict
...
class Drill:
    ...
    async def _fetch_html_playwright(
        ...
        async def set_geo_hook(context):
            """Hook to set proxy location before navigation."""
            if self.config.geolocation and hasattr(context, 'page'):
                try:
                    page = context.page
                    cdp = await page.context.new_cdp_session(page)
                    await cdp.send("Proxy.setLocation", self.config.geolocation)
                except Exception as e:
                    print(f"[DRILL] Failed to set proxy location: {e}")
        ...
        crawler = PlaywrightCrawler(
            ...
            pre_navigation_hooks=[set_geo_hook],
        )
