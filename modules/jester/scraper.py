...
class JesterMethod(Enum):
    ...
    APIFY = "apify"            # Apify Web Scraper API
...
class JesterConfig:
    ...
    enabled_methods: List[JesterMethod] = field(default_factory=lambda: [
        ...
        JesterMethod.APIFY,       # Apify Web Scraper (after Firecrawl, before BrightData)
...
class Jester:
    ...
    def __init__(self, config: Optional[JesterConfig] = None):
        ...
        # Apify Web Scraper
        self._apify_token = os.getenv("APIFY_API_TOKEN") or os.getenv("APIFY_TOKEN")
        self._apify_available = False
        self._apify_scraper = None
        ...
        # Initialize Apify if token available
        if self._apify_token:
            try:
                from modules.brute.engines.apify_scraper import ApifyWebScraper
                self._apify_scraper = ApifyWebScraper(self._apify_token)
                self._apify_available = True
            except Exception as e:
                logger.debug(f"Apify init skipped: {e}")
    ...
    async def scrape(
        ...
        Args:
            ...
            country: Optional ISO 3166-1 alpha-2 country code for Firecrawl proxy
            languages: Optional list of preferred languages for Firecrawl
        ...
        for m in methods:
            try:
                result = await self._try_method(url, m, country=country, languages=languages)
    ...
    async def _try_method(
        ...
        if method == JesterMethod.JESTER_A:
            ...
            if not self._firecrawl_key:
                return None
            return await self.scrape_firecrawl(url, country=country, languages=languages)
        elif method == JesterMethod.APIFY:
            if not self._apify_available:
                return None
            return await self.scrape_apify(url)
    ...
    async def scrape_apify(self, url: str) -> JesterResult:
        """
        APIFY: Apify Web Scraper Actor.
        
        Uses Apify's puppeteer-based web scraper for JS-heavy sites.
        Position: After Firecrawl, before BrightData
        """
        import time
        start = time.time()
        ...
        try:
            if not self._apify_available:
                return JesterResult(
                    url=url, html="", method=JesterMethod.APIFY,
                    latency_ms=0, status_code=0, content_length=0,
                    error="Apify not available"
                )
            
            # Use single-page scrape
            results = self._apify_scraper.scrape_url(url, max_pages=1, max_depth=0)
            latency = int((time.time() - start) * 1000)
            
            if results and results[0].get("text"):
                result = results[0]
                # Build HTML-like content from extracted data
                html = f"""<html>
<head><title>{result.get('title', '')}</title></head>
<body>
<h1>{result.get('h1', '')}</h1>
<div class="content">{result.get('text', '')}</div>
</body>
</html>"""
                ...
                return JesterResult(
                    url=url, html="", method=JesterMethod.APIFY,
                    latency_ms=latency, status_code=0, content_length=0,
