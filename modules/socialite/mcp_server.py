...
from modules.socialite.platforms import twitter, instagram, facebook, threads, linkedin
...
async def list_tools() -> List[Tool]:
    ...
    return [
        ...
        Tool(
            name="linkedin_login",
            description="Login to LinkedIn (Stubbed/Env-based).",
            inputSchema={
                "type": "object",
                "properties": {},
            }
        ...
        Tool(
            name="linkedin_profile_screenshot",
            description="Take a screenshot of a LinkedIn profile (via Firecrawl).",
            inputSchema={
                "type": "object",
                "properties": {
                    "username": {"type": "string", "description": "LinkedIn username or URL"}
                },
                "required": ["username"]
            }
        ),
...
async def call_tool(name: str, arguments: Any) -> List[TextContent]:
    ...
    try:
        if name == "search_twitter":
            ...
            driver = linkedin.LinkedInDriver()
            try:
        ...
        elif name == "linkedin_profile_screenshot":
            driver = linkedin.LinkedInDriver()
            try:
                result = await driver.screenshot_profile(arguments["username"])
                return [TextContent(type="text", text=json.dumps(result, indent=2))]
            finally:
                await driver.close()
