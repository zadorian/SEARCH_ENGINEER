...
================================================================================
C-1 STANDARD (2025-01)
================================================================================

C-1 outputs MUST be written to Elasticsearch via the CYMONIDES-1 indexer.
Local output files are already in C-1 shape; DO NOT stop at file writes.

Rules:
1. Use `ProjectIndexer.index_batch` with NodeCreator/EdgeCreator (c1/) to persist to `cymonides-1-{projectId}`.
2. Node IDs must be deterministic (SHA-256 of stable inputs). Never use Python `hash()`.
3. Every node/edge MUST include `source_module` + `source_scenario`.
   - `source_scenario` comes from the module's `scenarios.json`.
   - If a new flow is added, update that module's `scenarios.json` with a precise scenario id.
4. Edge-only outputs still require node ops for both endpoints before the edge.
5. IO Graph Persister is the reference pipeline:
   `/data/SEARCH_ENGINEER/BACKEND/modules/input_output/matrix/graph_persister.py`
...
### C-1 NODE + EDGE SCHEMA (Actual ES docs)
```json
{
  "id": "deterministic_hash",
  "kind": "person | company | domain | url | ...",
  "subkind": null,
  "node_class": "subject | location | nexus | narrative",
  "name": "Display Name",
  "comment": "Snippet / details (Column B)",
  "properties": {},
  ...
  "embedded_edges": [
    {
      "id": "edge_id",
      "relationship_type": "officer_of | has_email | links_to | ...",
      "source_id": "node_id",
      "target_id": "node_id",
      "source_kind": "person",
      "target_kind": "company",
      "direction": "outgoing",
      "category": "corporate | location | link | provenance | ...",
      "confidence": 0.85,
      "metadata": {},
      "source_module": "module_name",
      "source_scenario": "scenario-id",
...
async def cymonides_test_index_tool(args: Dict[str, Any]) -> Dict[str, Any]:
    """Index a test batch."""
    # Handle field_mappings as string or dict
    field_mappings = args["field_mappings"]
    if isinstance(field_mappings, str):
        try:
            field_mappings = json.loads(field_mappings)
        except json.JSONDecodeError:
            return {"content": [{"type": "text", "text": json.dumps({"error": "Invalid field_mappings JSON string"})}]}
...
class CymonicesAgent:
    ...
    def __init__(self, model: str = "claude-sonnet-4-20250514", interactive: bool = True):
        self.model = model
        self.interactive = interactive
        # Tool names for allowed_tools - use MCP namespaced format
        tool_names = [
            # C-3 Indexing Tools
            "mcp__cymonides__cymonides_load_sample",
            "mcp__cymonides__cymonides_get_indices",
            "mcp__cymonides__cymonides_test_index",
            "mcp__cymonides__cymonides_full_index",
            "mcp__cymonides__cymonides_status",
            # C-1 Bridge Building Tools
            "mcp__cymonides__cymonides_analyze_module",
            "mcp__cymonides__cymonides_generate_bridge",
            "mcp__cymonides__cymonides_save_bridge",
        ]
        self.options = ClaudeAgentOptions(
            ...
            permission_mode="acceptEdits",  # Accept edits, still prompts for tools unless in allowed_tools
        )

    async def run(self, task: str, input_callback=None) -> Dict[str, Any]:
        """
        ...
        6. ASK QUESTIONS if needed (interactive mode)

        Args:
            task: What to do (e.g., "Index /path/to/data.csv")
            input_callback: Optional async callback for getting user input
                           If None and interactive=True, uses stdin
        ...
        async with ClaudeSDKClient(options=self.options) as client:
            ...
            while not completed:
                    ...
                    if isinstance(message, AssistantMessage):
                        for block in message.content:
                            if isinstance(block, TextBlock):
                                ...
                                # Print in interactive mode so user sees the question
                                if self.interactive:
                                    print(block.text)
                ...
                # If not completed and interactive, check if agent is waiting for input
                if not completed and self.interactive:
                    # Check if the last response was a question (no pending tool calls)
                    if response_text and not any(
                        block for block in (message.content if hasattr(message, 'content') else [])
                        if isinstance(block, ToolUseBlock)
                    ):
                        # Get user input
                        if input_callback:
                            user_input = await input_callback(response_text)
                        else:
                            user_input = input("\n> ")
                        ...
                        if user_input.strip():
                            ...
                            completed = True  # Empty input means done
