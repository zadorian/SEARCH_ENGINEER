...
**Index Pattern:** `C-1-project-{PROJECT_ID}`

**Examples:**
- `C-1-project-ge13t70tq8v0hw0h994z1v64` (1,722 entities)
- `C-1-project-test` (7 entities)
- `C-1-project-y5p1ujpf31j4vtil3fi80hx5` (1 entity)
...
```
Project Narrative (Frontend) → C-1-project-{id} (Backend ES)
         ↓                                  ↓
...
**Code:** `server/services/C-2/EntityExtractor.ts`
...
1. User uploads PDF → Indexed in C-2 (`C-2`)
2. Claude Haiku extracts entities → Created in C-1 (`C-1-project-{id}`)
...
| `server/services/C-1/EdgeManager.ts` | Edge validation & creation |
| `server/services/C-1/schemas.ts` | C-1 ES mappings |
| `server/services/C-1/indexSetup.ts` | Index initialization |
...
| `server/services/C-2/EntityExtractor.ts` | Document extraction |
...
3. C-1 index created: C-1-project-{new_id}
...
- **C-2 Integration:** `../C-2/README.md`
- **Extraction Service:** `../../server/services/extraction/LLMExtractor.ts`
- **C-1 Metadata:** `../../metadata/C-1/README.md`
