...
# BrightData Archive - native cat{academic}! support (17.5 PB cached web data)
try:
...
class AcademicSearch:
    ...
    async def search(self, query: str, max_results: int = 100, tier: int = 1) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # BrightData Archive - native cat{academic}! support
            if BRIGHTDATA_ARCHIVE_AVAILABLE and search_by_category:
                try:
                    bd_results = await search_by_category(
                        category="academic",
                        limit=max_results // 4,  # Portion of total
                    )
                    for r in bd_results:
                        r['layer_used'] = 'BD_ARCHIVE'
                        r['source'] = 'brightdata_archive'
                    combined_results.extend(bd_results)
                    logger.info(f"BrightData Archive added {len(bd_results)} academic results")
                except Exception as e:
                    logger.warning(f"BrightData Archive academic search failed: {e}")
