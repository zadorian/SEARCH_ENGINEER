...
# BrightData Archive - native cat{ecommerce}! support (17.5 PB cached web data)
try:
...
class ProductSearch:
    ...
    async def search(self, query: str, max_results: int = 200) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # BrightData Archive - native cat{ecommerce}! support
            if BRIGHTDATA_ARCHIVE_AVAILABLE and search_by_category:
                try:
                    ...
                    for r in bd_results:
                        ...
                        r['search_type'] = 'product'
                        r['product_query'] = cleaned_query
                    all_results.extend(bd_results)
                    logger.info(f"BrightData Archive added {len(bd_results)} ecommerce results")
                except Exception as e:
                    logger.warning(f"BrightData Archive ecommerce search failed: {e}")
