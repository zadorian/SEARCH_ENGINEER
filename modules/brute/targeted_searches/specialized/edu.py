...
# BrightData Archive - native cat{education}! support (17.5 PB cached web data)
try:
...
class EduSearch:
    ...
    async def search(self, query: str, max_results: int = 200) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # BrightData Archive - native cat{education}! support
            if BRIGHTDATA_ARCHIVE_AVAILABLE and search_by_category:
                try:
                    ...
                    for r in bd_results:
                        ...
                        r['edu_query'] = cleaned_query
                    all_results.extend(bd_results)
                    logger.info(f"BrightData Archive added {len(bd_results)} education results")
                except Exception as e:
                    logger.warning(f"BrightData Archive education search failed: {e}")
