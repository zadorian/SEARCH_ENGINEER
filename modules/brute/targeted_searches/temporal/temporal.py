...
# Import date-native engines (NewsAPI, GDELT, Archive.org)
try:
    from newsapi import ExactPhraseRecallRunnerNewsAPI
    NEWSAPI_AVAILABLE = True
except ImportError as e:
    print(f"Warning: NewsAPI not available: {e}")
    NEWSAPI_AVAILABLE = False

try:
    from gdelt import ExactPhraseRecallRunnerGDELT
    GDELT_AVAILABLE = True
except ImportError as e:
    print(f"Warning: GDELT not available: {e}")
...
try:
    ...
    print(f"Warning: Archive.org not available: {e}")
    ARCHIVEORG_AVAILABLE = False

# BrightData Archive (17.5 PB cached web data) - date-native
try:
...
class BrightDataArchiveWrapper:
    """Wrapper for BrightData Archive to fit the engine pattern."""
    ...
    def client(self):
        if self._client is None:
            self._client = BrightDataArchive()
        return self._client
    ...
    async def search(
        ...
        """Search BrightData Archive with date/lang/geo/cat filters."""
        import asyncio
        ...
        for _ in range(15):  # 30 seconds max
            ...
            status = await self.client.get_search_status(search_id)
            if status.get("ready"):
                ...
                return [{
                    ...
                    "dump_cost_usd": status.get("dump_cost_usd"),
                    "query": query,
                    "requires_dump": True,
                }]
            if "error" in status:
...
class DateSearchRouter(SearchTypeEventEmitter):
    ...
    def _initialize_engines(self) -> Dict[str, Any]:
        ...
        # Date-native engines (NewsAPI, GDELT, Archive.org)
        if NEWSAPI_AVAILABLE:
            try:
                engines['newsapi'] = 'newsapi'  # Marker - will be instantiated per-query with dates
                logger.info("Initialized NewsAPI (date-native)")
            except Exception as e:
                logger.warning(f"Could not initialize NewsAPI: {e}")

        if GDELT_AVAILABLE:
            try:
                engines['gdelt'] = 'gdelt'  # Marker - will be instantiated per-query with dates
                logger.info("Initialized GDELT (date-native)")
            except Exception as e:
                logger.warning(f"Could not initialize GDELT: {e}")

        if ARCHIVEORG_AVAILABLE:
            try:
                engines['archiveorg'] = ArchiveOrgSearch()
                logger.info("Initialized Archive.org (date-native)")
            except Exception as e:
                logger.warning(f"Could not initialize Archive.org: {e}")

        if BRIGHTDATA_AVAILABLE:
            try:
                engines['brightdata'] = BrightDataArchiveWrapper()
                logger.info("Initialized BrightData Archive (date/lang/geo/cat-native)")
            except Exception as e:
                logger.warning(f"Could not initialize BrightData Archive: {e}")
    ...
    def translate_to_engine_syntax(self, date_info: Dict[str, Any], keywords: str, engine: str) -> str:
        ...
        if engine == 'google':
            ...
            # NewsAPI uses from/to parameters - handled via API params, not query string
            # Return just keywords; dates are passed separately to the API
            return keywords

        elif engine == 'gdelt':
            # GDELT uses STARTDATETIME/ENDDATETIME - handled via API params
            # Return just keywords; dates are passed separately to the API
            return keywords

        elif engine == 'archiveorg':
            # Archive.org uses date:[YYYY-MM-DD TO YYYY-MM-DD] in query
            if date_info['type'] == 'before':
                return f'{keywords} date:[* TO {date_info["end_date"]}]'
            elif date_info['type'] == 'after':
                return f'{keywords} date:[{date_info["start_date"]} TO *]'
            elif date_info['type'] in ['range', 'year', 'month', 'exact']:
                return f'{keywords} date:[{date_info["start_date"]} TO {date_info["end_date"]}]'
    ...
    def search(self, query: str, output_file: str = None) -> Dict[str, Any]:
        ...
        for engine_name, engine in self.engines.items():
            ...
            try:
                # Handle date-native engines specially (NewsAPI, GDELT)
                if engine_name == 'newsapi' and NEWSAPI_AVAILABLE:
                    # NewsAPI uses from/to API parameters
                    newsapi_runner = ExactPhraseRecallRunnerNewsAPI(
                        phrase=keywords,
                        start_date=date_info.get('start_date'),
                        end_date=date_info.get('end_date')
                    )
                    engine_results = newsapi_runner.search(keywords, max_results=50)
                    print(f"  [L1] NewsAPI native date search: {len(engine_results)} results")

                elif engine_name == 'gdelt' and GDELT_AVAILABLE:
                    # GDELT uses STARTDATETIME/ENDDATETIME
                    # Calculate timespan for GDELT
                    gdelt_timespan = "3m"  # Default 3 months
                    if date_info['type'] == 'year':
                        gdelt_timespan = "12m"
                    elif date_info['type'] == 'month':
                        gdelt_timespan = "1m"

                    gdelt_runner = ExactPhraseRecallRunnerGDELT(
                        phrase=keywords,
                        timespan=gdelt_timespan,
                        num_time_slices=2
                    )
                    # Collect results from generator
                    for article in gdelt_runner.run():
                        engine_results.append(article.to_dict())
                        if len(engine_results) >= 50:
                            break
                    print(f"  [L1] GDELT native date search: {len(engine_results)} results")

                else:
                    # Standard engines - use query variations
                ...
                # Apply filtering if enabled (common to all engines)
                ...
                results['engines'][engine_name] = {
                    'query': variations[0][0] if variations else keywords,
