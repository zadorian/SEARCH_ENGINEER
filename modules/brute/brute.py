...
# Import PACMAN processor for entity extraction on streamed results
try:
    from brute.pacman_processor import process_brute_result, get_result_entities
    PACMAN_PROCESSOR_AVAILABLE = True
except ImportError:
    try:
        from pacman_processor import process_brute_result, get_result_entities
        PACMAN_PROCESSOR_AVAILABLE = True
    except ImportError:
        PACMAN_PROCESSOR_AVAILABLE = False
        process_brute_result = None
        get_result_entities = None

# Multi-backend search for Google/Bing (SerpAPI + Direct API)
try:
    from engines.multi_backend import MultiBackendSearch
...
# Import Cymonides categoriser for ES-based domain lookup
try:
    from categoriser import categorise_url, extract_entities_simple, has_extractable_data, SourceCategory
    CYMONIDES_CATEGORISER_AVAILABLE = True
except ImportError:
    CYMONIDES_CATEGORISER_AVAILABLE = False
    categorise_url = None
    extract_entities_simple = None
    has_extractable_data = None
    SourceCategory = None
...
ENGINE_PERFORMANCE = {
    'fast': ['CY', 'GO', 'FC', 'BI', 'BR', 'EX', 'DD', 'YA', 'OA', 'CR', 'OL', 'PM', 'AX', 'SE', 'WP', 'BK', 'YO', 'AP'],     # 30 second timeout - Yandex & SocialSearcher moved to fast tier, You.com added
    'medium': ['GD', 'AL', 'PW', 'GR', 'NA', 'HF', 'GU', 'W', 'NT', 'JS', 'MU', 'SG', 'AA', 'LG', 'BA', 'QW', 'SB'],   # 60 second timeout - Academic scraping engines, book repos, Baidu, Qwant, SerpBrightData
...
def create_google_client():
    ...
    from modules.brute.engines.google import GoogleSearch
    return GoogleSearch()

def create_bing_client():
    """Create BingSearch instance"""
    from modules.brute.engines.bing import BingSearch
    return BingSearch()

def create_brave_client():
    """Create BraveSearch instance"""
    from modules.brute.engines.brave import BraveSearch
    return BraveSearch()

def create_yandex_client():
    """Create YandexSearch instance"""
    from modules.brute.engines.yandex import YandexSearch
    return YandexSearch()

def create_yep_scraper():
    """Create YepScraper instance"""
    from modules.brute.engines.yep import YepScraper
    return YepScraper()

def create_archiveorg_client():
    """Create ArchiveOrgSearch instance"""
    from modules.brute.engines.archiveorg import ArchiveOrgSearch
    return ArchiveOrgSearch()

def create_qwant_scraper():
    """Create QwantScraper instance"""
    from modules.brute.engines.qwant_scraper import QwantScraper
    return QwantScraper()

def create_baidu_scraper():
    """Create BaiduScraper instance"""
    from modules.brute.engines.baidu import BaiduScraper
    return BaiduScraper()

def create_serp_brightdata_client():
    """Create SerpBrightData instance"""
    from modules.brute.engines.serp_brightdata import SerpBrightData
    return SerpBrightData()

# Engine configuration with two-letter codes
ENGINE_CONFIG = {
    'SB': {
        'name': 'SerpBrightData',
        'module': 'engines.serp_brightdata',
    ...
    'AR': {
        ...
        'supports_streaming': False,
        'init_kwargs': lambda: {'archive': create_archiveorg_client()}
    },
    'AP': {
        'name': 'Apify SERP',
        'module': 'engines.apify_serp',
        'class': 'ExactPhraseRecallRunnerApify',
        'supports_streaming': False
    },
    'GI': {
        'name': 'Google Images (Apify)',
        'module': 'engines.apify_images',
    ...
    'AL': {
        ...
        'init_kwargs': lambda: {
            'include_related': True,        # Fetch related entities for max recall
            'max_results_per_query': 200,   # Max results
...
WRAPPER_ENGINE_MAP = {
    ...
    'SB': ('engines.serp_brightdata', 'SerpBrightData'),
}
...
class BruteSearchEngine:
    ...
    def __init__(self, keyword: str, output_file: str = None, 
                 ...
                 checkpoint_id: Optional[str] = None, event_emitter=None, return_results: bool = False, firecrawl_full_scrape: bool = True):
        self.keyword = keyword
        self.event_emitter = event_emitter  # For WebSocket events
        self._search_start_time = None
        self.return_results = return_results
        self.final_results = [] if return_results else None
        self.firecrawl_full_scrape = firecrawl_full_scrape
        ...
        if engines:
            ...
            fast_engines = ['GO', 'FC', 'BI', 'BR', 'DD', 'YA', 'YO', 'GR', 'NA', 'GD', 'OA', 'CR', 'SE', 'WP', 'AP', 'EX']  # EX last for exclusions  # EX last to get exclusions from others  # GO first for instant results, FC second to warm up  # Fast APIs - Brave and Exa ARE fast!
            medium_engines = ['HF', 'AL', 'PW', 'BO', 'W', 'QW', 'BA', 'BK', 'OL', 'PM', 'AX', 'GU', 'SB', 'GI']  # BA=Baidu  # Moderate complexity
            slow_engines = ['AR', 'YE', 'NT', 'JS', 'MU', 'SG', 'AA', 'LG']  # Heavy scraping  # Heavy processing/delays
    ...
    def _cached_categorize(self, url: str, title: str, snippet: str):
        """
        Cached domain categorization with Cymonides lookup.
        Priority: Cymonides ES -> basic heuristics -> GPT fallback.
        """
        try:
            ...
            # Try Cymonides lookup first (ES-based domain intelligence)
            if CYMONIDES_CATEGORISER_AVAILABLE and categorise_url:
                try:
                    cymo_result = categorise_url(url)
                    if cymo_result and cymo_result.best_category:
                        category_result = {
                            'category': cymo_result.best_category,
                            'subcategory': cymo_result.io_category,
                            'filetype': None,
                            'country': None,
                            'attributes': {
                                'source': 'cymonides',
                                'io_quality_tier': cymo_result.io_quality_tier,
                                'io_source_type': cymo_result.io_source_type,
                            }
                        }

                        # Queue entity extraction if rich data found
                        if has_extractable_data and has_extractable_data(cymo_result.raw_data):
                            self._queue_entity_extraction(domain, cymo_result.raw_data)
                    ...
                    logger.debug(f"Cymonides lookup failed for {domain}: {e}")
    ...
    def _queue_entity_extraction(self, domain: str, raw_data: dict):
        """Queue entity extraction for domains with rich data."""
        if not hasattr(self, 'entity_extraction_queue'):
            self.entity_extraction_queue = []

        # Only queue if we haven't processed this domain yet
        if not hasattr(self, 'extracted_domains'):
            self.extracted_domains = set()

        if domain not in self.extracted_domains and len(self.entity_extraction_queue) < 100:
            self.extracted_domains.add(domain)
            self.entity_extraction_queue.append({
                'domain': domain,
                'raw_data': raw_data
            })

            # Process in batches of 10
            if len(self.entity_extraction_queue) >= 10:
                self._process_entity_extraction_queue()

    def _process_entity_extraction_queue(self):
        """Process queued entity extractions."""
        if not self.entity_extraction_queue:
            return

        if extract_entities_simple:
            import threading
            def extract_batch():
                batch = self.entity_extraction_queue[:10]
                self.entity_extraction_queue = self.entity_extraction_queue[10:]

                for item in batch:
                    try:
                        result = extract_entities_simple(item['domain'], item['raw_data'])
                        if result.nodes:
                            logger.info(f"Extracted {len(result.nodes)} entities from {item['domain']}")
                            # Emit entity event to frontend
                            self.emit_event_sync('entities_extracted', {
                                'domain': item['domain'],
                                'nodes': [n.__dict__ for n in result.nodes],
                                'edges': [e.__dict__ for e in result.edges]
                            })
                    except Exception as e:
                        logger.debug(f"Entity extraction failed for {item['domain']}: {e}")
    ...
    def _run_engine(self, engine_code: str) -> Iterator[Dict[str, Any]]:
        ...
        try:
            ...
            for query_idx, current_query in enumerate(queries_to_run):
                ...
                # MULTI-BACKEND: Use combined SerpAPI + Direct API for Google/Bing exact phrase searches
# BYPASS REMOVED:                 is_exact_phrase = current_query.startswith('"') or self.force_exact_phrase
# BYPASS REMOVED:                 if MULTI_BACKEND_AVAILABLE and engine_code in ['GO', 'BI'] and is_exact_phrase:
# BYPASS REMOVED:                     try:
# BYPASS REMOVED:                         mb = MultiBackendSearch()
# BYPASS REMOVED:                         if engine_code == 'GO':
# BYPASS REMOVED:                             mb_results = asyncio.run(mb.search_google(current_query, max_results=100))
# BYPASS REMOVED:                         else:  # BI
# BYPASS REMOVED:                             mb_results = asyncio.run(mb.search_bing(current_query, max_results=100))
# BYPASS REMOVED:                         if mb_results:
# BYPASS REMOVED:                             results = mb_results
# BYPASS REMOVED:                             used_wrapper = True
# BYPASS REMOVED:                             logger.info(f"[{engine_code}] Multi-backend: {len(results)} results from SerpAPI + API")
# BYPASS REMOVED:                             print(f"✓ [{engine_code}] Multi-backend: {len(results)} results")
# BYPASS REMOVED:                     except Exception as e:
# BYPASS REMOVED:                         logger.warning(f"Multi-backend failed for {engine_code}, falling back: {e}")
# BYPASS REMOVED: 
                try:
                ...
                if not used_wrapper:
                    ...
                    # SPECIAL: For Exa, pass already-found domains as exclusions
                    if engine_code == 'EX' and hasattr(self, 'deduplicator') and self.deduplicator:
                        try:
                            conn = self.deduplicator.conn
                            cursor = conn.cursor()
                            cursor.execute("SELECT url FROM seen_urls LIMIT 500")
                            found_urls = [row[0] for row in cursor.fetchall()]
                            from urllib.parse import urlparse
                            found_domains = list(set(urlparse(u).netloc.lower() for u in found_urls if u))
                            if found_domains:
                                kwargs['initial_exclude_domains'] = found_domains[:100]
                                logger.info(f"[EX] Passing {len(found_domains[:100])} domains to exclude")
                        except Exception as e:
                            logger.debug(f"Could not get exclusion domains for Exa: {e}")
                    ...
                    if engine_code == 'GO':
                        ...
                        # Firecrawl __init__ only takes api_key
                        kwargs.pop('phrase', None)
                    ...
                    if engine_code == 'DD':
                        ...
                        # Firecrawl run() expects phrase as first argument
                        phrase_for_fc = current_query.strip('"')
                        fc_tier = 3 if self.firecrawl_full_scrape else 1
                        results = runner.run(phrase=phrase_for_fc, tier=fc_tier)
                    elif engine_code == 'BO':
    ...
    def _result_writer_thread(self):
        ...
        while not self._stop_processing or not self.result_queue.empty() or not self.priority_queue.empty():
            try:
                ...
                if self.return_results and result and not self.event_emitter:
                ...
                if action == 'new':
                    ...
                    # PACMAN PROCESSOR: Queue for scraping + entity extraction
                    # Extracts: persons, companies, locations, dates, identifiers, contacts
                    if PACMAN_PROCESSOR_AVAILABLE and process_brute_result:
                        try:
                            process_brute_result(result)
                        except Exception as e:
                            logger.debug(f"PACMAN processor queue failed: {e}")
    ...
    def _filtered_writer_thread(self):
        ...
        while not self._stop_processing or not self.filtered_out_queue.empty():
            try:
                ...
                # Also collect for return if needed
                if self.return_results and self.final_results is not None:
                    self.final_results.append(result)
    ...
    def _flush_batch(self, batch: List[tuple]):
        ...
        if self.return_results and not self.event_emitter:
        ...
        for result, action in batch:
            try:
                ...
                if self.return_results and self.final_results is not None:
                    self.final_results.append(result)
                logger.info(f"EMIT: result event for {result['url'][:50]}")
    ...
    def search(self):
        ...
        self._apply_flexible_exact_phrase_filter()
    ...
    async def _process_scraping_batch(self):
        ...
        try:
            ...
            # SKIP Firecrawl results that already have full content
            batch_to_scrape = [item for item in batch if not (item.get("_firecrawl_scraped") or item.get("_full_content") or item.get("markdown"))]
            skipped = len(batch) - len(batch_to_scrape)
            if skipped > 0:
                print(f"Skip {skipped} Firecrawl results (already scraped)")
            if not batch_to_scrape:
                return
            batch = batch_to_scrape  # Use filtered batch
    ...
    def _apply_flexible_exact_phrase_filter(self):
        """Filter results - flexible exact phrase matching (spaces can be - _ . / or nothing)"""
        phrase = self.keyword.strip('"').strip("'")
        ...
        passed, filtered = [], []
        for r in self.all_results_list:
            text = (r.get('title', '') or '') + ' ' + (r.get('snippet', '') or '') + ' ' + (r.get('url', '') or '')
            if regex.search(text):
                passed.append(r)
            else:
                r['filter_reason'] = f'No exact phrase: {phrase}'
                filtered.append(r)
        orig = len(self.all_results_list)
        self.all_results_list = passed
        self.filtered_results_list.extend(filtered)
        self.stats['filtered_results'] = self.stats.get('filtered_results', 0) + len(filtered)
        self.stats['filter_rate'] = round(len(filtered) / orig * 100, 1) if orig > 0 else 0
        logger.info(f'Exact phrase filter: {len(passed)} passed, {len(filtered)} filtered')
        print(f'🔍 Exact phrase filter: {len(passed)} passed, {len(filtered)} filtered ({self.stats.get("filter_rate", 0)}%)')
