...
class BruteSearchOptimal:
    ...
    def _load_default_registry(self) -> Dict[str, Any]:
        ...
        try:
            # Import from brute.py (Corrected Path)
            try:
                from modules.brute.brute import ENGINE_CONFIG
            except ImportError:
                # Fallback for relative import or different structure
                from .brute import ENGINE_CONFIG
            ...
            for code, config in ENGINE_CONFIG.items():
                ...
                try:
                    ...
                    if module_name and class_name:
                        ...
                        import importlib
                        try:
                            # Try full path first (module_name already has engines. prefix)
                            module = importlib.import_module(f"modules.brute.{module_name}")
                        except ImportError:
                        ...
                        # Create adapter for CascadeExecutor interface
                        kw_func = config.get('init_kwargs')
                        
                        class AdaptedRunner:
                            def __init__(self, phrase, max_results):
                                kwargs = kw_func() if callable(kw_func) else (kw_func or {}).copy()
                                kwargs['phrase'] = phrase
                                # Adapt max_results -> max_results_per_query for ExactPhraseRecall runners
                                kwargs['max_results_per_query'] = max_results
                                self.runner = runner_class(**kwargs)
                                self.engine_name = getattr(self.runner, 'engine_name', code)

                            def run(self):
                                return self.runner.run()

                        registry[code] = AdaptedRunner
