#!/usr/bin/env python3
"""
FORENSIC OPENAI SEARCH ENGINE
=============================
OpenAI GPT with forensic search methodology and web browsing.
Same depth-prioritized scoring as Forensic Gemini.

Engine Code: AI-OA (AI-OpenAI)
"""
...
# =============================================================================
# FORENSIC OPENAI CLIENT
# =============================================================================

class ForensicOpenAIClient:
    """OpenAI client with forensic search methodology."""
    ...
    def search_forensic(self, target: str, company: str = None, role: str = None,
        ...
        try:
            # Use GPT-4 with web browsing capability (or gpt-4-turbo)
            # Note: For actual web search, we use the responses API with web_search tool
            response = self.client.responses.create(
                model="gpt-4.1",  # Latest GPT with web search
                tools=[{"type": "web_search"}],
                input=f"""{FORENSIC_MASTER_PROMPT}
...
Return valid JSON array of results."""
            ...
            logger.error(f"OpenAI forensic search error: {e}")
    ...
    def _fallback_search(self, target: str, company: str = None, role: str = None,
                         max_results: int = 50) -> List[Dict]:
        """Fallback using standard chat completions."""
        try:
            context_parts = [target]
            if company:
                context_parts.append(company)
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": FORENSIC_MASTER_PROMPT},
                    {"role": "user", "content": f"""Generate forensic search queries and hypothetical results for: {target}
...
Return {max_results} hypothetical but realistic results."""}
            ...
            logger.error(f"OpenAI fallback search error: {e}")
            return []
...
class ExactPhraseRecallRunnerForensicOpenAI:
    """
    Forensic OpenAI Search Runner for Brute Search Integration.
    ...
    def __init__(
        ...
        if OPENAI_AVAILABLE:
            try:
                self.client = ForensicOpenAIClient()
            except Exception as e:
...
def forensic_search_openai(
    phrase: str,
    company: str = None,
    role: str = None,
    max_results: int = 100
) -> List[Dict]:
    """Quick forensic search using OpenAI."""
    runner = ExactPhraseRecallRunnerForensicOpenAI(
        phrase=phrase,
        company=company,
        role=role,
...
FORENSIC_OPENAI_AVAILABLE = OPENAI_AVAILABLE
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        target = sys.argv[1]
        print(f"Forensic OpenAI search for: {target}")
        results = forensic_search_openai(target)
        for r in results[:10]:
            print(f"  [{r.get('forensic_score', 0)}] {r.get('url', 'N/A')}")
    else:
        print("Usage: python exact_phrase_recall_runner_forensic_openai.py <target>")
