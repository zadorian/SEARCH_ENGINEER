...
# JESTER for JS-rendered scraping
try:
    ...
    def search(self, query: str, max_results: int = 50) -> List[Dict]:
        ...
        while len(all_results) < max_results:
            try:
                ...
                # Use JESTER with JS rendering for Baidu (heavy anti-bot)
                if JESTER_AVAILABLE and jester_scrape_sync:
                    html = jester_scrape_sync(url, force_js=True, timeout=30)
                    if not html:
                        logger.warning("JESTER failed, falling back to requests")
...
class ExactPhraseRecallRunnerBaidu:
    ...
    def _build_query_variations(self) -> List[str]:
        ...
        # BD1-BD4 AUTO-VARIATIONS for max recall
        quoted = base_query
        auto_queries = {
            "BD1_exact": quoted,
            "BD2_pdf": f"{quoted} filetype:pdf",
            "BD2_doc": f"{quoted} (filetype:doc OR filetype:docx)",
            "BD2_xls": f"{quoted} (filetype:xls OR filetype:xlsx)",
            "BD2_ppt": f"{quoted} (filetype:ppt OR filetype:pptx)",
            "BD3_intitle": f"intitle:{quoted}",
            "BD4_inurl": f"inurl:{quoted}",
        }
        variations.extend(auto_queries.values())
