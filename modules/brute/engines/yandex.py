...
class YandexSearch(SearchEngine):
    ...
    def search(self, query: str, max_results: int = 30) -> List[Dict]:
        """
        Standard search method with multi-backend support for max recall.
        ...
        """
        # MULTI-BACKEND: Use SerpAPI + BrightData + Native API
        if os.getenv('USE_MULTI_BACKEND', 'true').lower() == 'true':
            try:
                from .multi_backend_integration import multi_backend_yandex
                return multi_backend_yandex(query, max_results, 
                                           native_search_func=self.yandex_base)
