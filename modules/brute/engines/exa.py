...
class ExactPhraseRecallRunnerExa:
    ...
    def __init__(
        ...
        initial_exclude_domains: list = None,  # Domains from other engines to exclude
    ):
        self.phrase = phrase.strip('\'"') # Ensure phrase is unquoted internally
        
        # Domains to exclude from other engines
        self.initial_exclude_domains = initial_exclude_domains or []
        if self.initial_exclude_domains:
            logger.info(f"Exa starting with {len(self.initial_exclude_domains)} domains pre-excluded")
    ...
    def run_exception_search(self) -> List[Dict]:
        ...
        for i in range(0, len(all_domains), max_domains_per_query):
            ...
            try:
                ...
                if self.initial_exclude_domains:
                    existing = params.get('exclude_domains', [])
                    params['exclude_domains'] = list(set(existing + self.initial_exclude_domains[:100]))
