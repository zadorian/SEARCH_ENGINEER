...
class ExactPhraseRecallRunnerPerplexity:
    ...
    def run(self, phrase: str, tier: int = 1) -> List[Dict]:
        ...
        if tier == 1:
            ...
            all_results = []
            # PP1-PP4 variations for max recall
            quoted = f'"{phrase}"' if not phrase.startswith('"') else phrase
            queries = [
                quoted,  # PP1: Exact
                f"{quoted} filetype:pdf",  # PP2: PDF documents
                f"{quoted} filetype:doc",  # PP2: Word docs
                f"intitle:{quoted}",  # PP3: In title
                f"inurl:{quoted}",  # PP4: In URL
                f"{phrase} site:gov OR site:edu",  # PP5: Authoritative
