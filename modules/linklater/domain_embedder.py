...
Features:
- OpenAI text-embedding-3-large (3072 dims) for multilingual support
...
EMBEDDING_MODEL = "text-embedding-3-large"  # 3072 dims, multilingual
EMBEDDING_DIMS = 3072
...
class DomainEmbedder:
    ...
    Uses OpenAI text-embedding-3-large for high-dimensional multilingual embeddings.
    ...
    async def _ensure_index(self):
        """Ensure cymonides-2 has the OpenAI embedding field for domain content."""
        ...
        if not exists:
            ...
            mappings = {
                "mappings": {
                    "properties": {
                        ...
                        # Embeddings - both MiniLM (384) and OpenAI (3072)
                        "content_embedding": {
                            "type": "dense_vector",
                            "dims": 384,
                            "index": True,
            ...
            # Add OpenAI embedding field if missing
            try:
                ...
                if "content_embedding_openai" not in props:
                    ...
                    logger.info("Added OpenAI embedding field to cymonides-2")
    ...
    async def embed_content(
        ...
        for i, (chunk, embedding) in enumerate(zip(chunks, embeddings)):
            ...
            doc = {
                ...
                # OpenAI embedding (3072 dims, multilingual)
                "content_embedding_openai": embedding,
                    ...
                    "should": [
                        {
                            "script_score": {
                                "query": {"match_all": {}},
                                "script": {
                                    "source": "cosineSimilarity(params.query_vector, 'content_embedding_openai') + 1.0",
    ...
    async def get_url_versions(
        ...
        if include_embeddings:
            source_fields.append("content_embedding_openai")
        ...
        try:
            ...
            for hit in result["hits"]["hits"]:
                ...
                if include_embeddings and "content_embedding_openai" in src:
                    v["embedding"] = src["content_embedding_openai"]
                versions.append(v)
        ...
        try:
            target_result = await self.es_client.search(
                ...
                body={
                    ...
                    "size": 1,
                    "_source": ["content_embedding_openai"]
                }
            )
            ...
            target_embedding = target_result["hits"]["hits"][0]["_source"].get("content_embedding_openai")
            ...
            for hit in candidates["hits"]["hits"]:
                ...
                if cand_result["hits"]["hits"]:
                    cand_emb = cand_result["hits"]["hits"][0]["_source"].get("content_embedding_openai")
                    if cand_emb:
        ...
        async def get_doc_embedding(url: str, timestamp: Optional[str]) -> Optional[tuple]:
            ...
            result = await self.es_client.search(
                ...
                body={
                    ...
                    "sort": sort,
                    "size": 1,
                    "_source": ["content_embedding_openai", "content", "archive_timestamp", "domain"]
                }
            )
            ...
            if result["hits"]["hits"]:
                ...
                return (
                    src.get("content_embedding_openai"),
                    src.get("content", "")[:500],
        ...
        try:
            result = await self.es_client.search(
                ...
                body={
                    ...
                    "sort": [{"archive_timestamp": "asc"}],
                    "size": 100,
                    "_source": ["content", "content_embedding_openai", "archive_timestamp", "scrape_source"]
                }
            )
            ...
            for hit in result["hits"]["hits"]:
                ...
                versions.append({
                    ...
                    "embedding": src.get("content_embedding_openai"),
                    "timestamp": src.get("archive_timestamp"),
                    "source": src.get("scrape_source"),
                })
