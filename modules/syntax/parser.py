...
class OperatorType(Enum):
    """Types of operators for execution routing."""
    # Entity extraction
    ENTITIES_ALL = "ent?"
    PERSON = "p?"
    COMPANY = "c?"
    ...
    # IO Prefix (entity investigation)
    IO_PERSON = "p:"
    IO_COMPANY = "c:"
    IO_EMAIL = "e:"
    IO_PHONE = "t:"
...
@dataclass
class GridTarget:
    """A grid node target for queries."""
    node_id: str              # The node ID in the grid
    expanded: bool = True     # True = include connected nodes, False = node only

    @classmethod
    def from_target(cls, target: 'Target') -> 'GridTarget':
        """Create GridTarget from a parsed Target."""
        return cls(
...
OPERATOR_TO_SUBJECT_SIDE: Dict[str, str] = {
    # Entity extraction operators - short forms (@ prefix optional)
...
class ParsedQuery:
    ...
    wants_backlinks: bool = False
    wants_outlinks: bool = False
    ...
    def __post_init__(self):
        ...
        for op in self.operators:
            ...
            elif op_def.category == OperatorCategory.QUERY:
                self.wants_enrichment = True
            elif op_def.category == OperatorCategory.IO_PREFIX:
                self.wants_extraction = True  # IO prefixes trigger entity search
        ...
        # Specific operator flags
        for op in self.operators:
            op_lower = op.lower()
            if op_lower in ("bl?", "bl!", "backlinks"):
                self.wants_backlinks = True
            elif op_lower in ("ol?", "ol!", "outlinks"):
                self.wants_outlinks = True
            elif op_lower in ("ent?", "@ent?", "entities"):
                self.wants_entities = True
...
class SyntaxParser:
    ...
    def _parse_web_query(
        ...
        # Handle IO prefix operators (p:, c:, e:, t:, d:, u:, li:) - colon is part of operator
        # These are entity investigation queries like "p: John Smith :US"
        io_prefix_match = re.match(r"^([pcedtuPCEDTU]|li|LI):\s*(.*)$", query, re.IGNORECASE)
        if io_prefix_match:
            io_operator = io_prefix_match.group(1).lower() + ":"  # e.g., "p:"
            target_str = io_prefix_match.group(2).strip()

            # Extract jurisdiction suffix (:XX) - adds to dimension_filters
            jur_match = re.search(r"\s+:([A-Z]{2})\s*$", target_str, re.IGNORECASE)
            if jur_match:
                dimension_filters = list(dimension_filters) + [
                    DimensionFilter(dimension="jurisdiction", value=jur_match.group(1).upper())
                ]
                target_str = target_str[:jur_match.start()].strip()

            # For IO prefixes, the target is the entity name (not a domain)
            # Remove quotes if present
            target = target_str.strip().strip('"').strip("'")

            return self._build_web_result(
                [io_operator], target, TargetType.WEB_DOMAIN, HistoricalRange(),
