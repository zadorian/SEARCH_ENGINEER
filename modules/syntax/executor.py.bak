...
IO_MATRIX_DIR = DATA_ROOT / "INPUT_OUTPUT" / "matrix"
...
class UnifiedExecutor:
    ...
    async def _execute_brute(self, query: str, project_id: str) -> Dict[str, Any]:
        """
        Execute via BruteSearch with TLD filter enhancement.
        ...
        if has_tld_filter(query):
            tld_filters, search_query = parse_tld_filter(query)
            ...
            for tld in tld_filters:
                ...
                try:
                    ...
                    engines = get_tld_engine_tiers(tld)
                    engine_filters.extend(engines)
                except ImportError:
                    if tld == "news!":
            ...
            logger.info(f"  TLD filters: {tld_filters}, Sites: {site_modifiers}, Engines: {engine_filters}")
        else:
            logger.info(f"BruteSearch Execute: {query}")

        try:
            # Pass query to BruteSearch
            # If engine_filters specified, pass as engine selection hint
            if engine_filters:
                results = await brute.search_async(
                    search_query,
                    engines=engine_filters  # Hint to prioritize these engines
                )
            else:
            ...
            # BruteSearch may not support engines param - fallback
            try:
                results = await brute.search_async(search_query)
                results["_executor"] = "brute"
                results["_project_id"] = project_id
